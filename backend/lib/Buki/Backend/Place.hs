...
addPlace _ pName = do
  pId <- dbMkUuid
  p <- dbCatch (onAnyUniqueViolation (mkFailure PlaceNameTaken))
      $ mkSuccess <$> dbInsert1 M.placeTable (O.toFields M.Place
          { M.place'Id = pId
          , M.place'Name = pName
          }) id
  pure $ toPlaceData <$> p
...
removeUnusedPlace _ pId = do
  (fmap . fmap) toPlaceData $
    dbCatch (onAnyForeignKeyViolation PlaceStillUsed) $
      dbDelete1 M.placeTable (\p -> p ^. M.id .== O.toFields pId) id
      >>= liftE @'[ PlaceNotFound, PlaceStillUsed ] (\NoRecordsFound -> pure $ Left PlaceNotFound)
