...
assertError :: forall err errs a es. (IOE :> es, err `In` errs, Eq err, Show err, Show (Union errs)) => err -> Err errs a -> Eff es ()
assertError err (Failure e) = case project @err e of
  (Just err') -> if err == err'
                 then pure ()
                 else liftIO $ error $ "Expected error " <> show err <> ", but got "
                                                         <> show err' <> "!"
  _ -> liftIO $ error $ "Expected error " <> show err <> ", but got " <> show e <> "!"
